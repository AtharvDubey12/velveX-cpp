#include "../headers/diffHandler.h"
#include "../headers/expressionParser.h"
#include <unordered_map>
#include <unordered_set>
using namespace std;
//Diff[1][x][x]
string diffHandler(vector<string>& tokenList){
    int degree = 1;
    string innerFunc = "";
    ...
    if(degree>1) num_sanitized ="\\frac{d^" + to_string(degree) +" "+ innerFunc + "}";
    else num_sanitized ="\\frac{d" + innerFunc + "}";
    string den_sanitized = "{";

    for(int i = 0; i<varList.size(); i++){
        if(i==varList.size()-1 && degree>1)
            den_sanitized += " d" + varList[i]+"^"+ to_string(degree);
        else if (!i)
            den_sanitized += "d" + varList[i];
        else
            den_sanitized += " d" + varList[i];
        degree--;
    }
